...
export const NETWORKS = {
  ...
  HEDERA_TESTNET: {
    chainId: 296,
    name: "Hedera Testnet",
    currency: { name: "Hedera", symbol: "HBAR", decimals: 18 }, // Explicitly define currency for Hedera
    rpcUrl: process.env.NEXT_PUBLIC_HEDERA_TESTNET_RPC_URL || "https://testnet.hashio.io/api", // Updated RPC URL
...
export const addNetworkToMetamask = async (network: (typeof NETWORKS)[keyof typeof NETWORKS]) => {
  ...
  const params: any = {
    ...
    nativeCurrency: network.currency || { name: "Ether", symbol: "ETH", decimals: 18 }, // Default to ETH if not specified
